...
def get_meso_metadata(token, terrpath, marpath):
    try:
        ...
        url = "https://api.synopticdata.com/v2/stations/metadata?token={}&network=65&bbox={}&recent=20&output=json".format(
            token, bbox_api
        )
        ...
        for each in request["STATION"]:
            ids.append(
                [each["STID"], each["PERIOD_OF_RECORD"]["start"]]
            )  # Keep station ID and start date

        ids = pd.DataFrame(ids, columns=["STID", "start"]).sort_values(
            "start"
...
def get_cwop_station_csv(
            ...
            with requests.get(url, stream=True) as r:
                if r.status_code == 200:  # If API call returns a response
                    if (
                        ...
                        print(
                            "Saving data for station {}".format(id["STID"])
                        )  # Nice for testing, remove for full run.
    ...
    s3_cl.put_object(
        Bucket=bucket_name,
        Body=content,
        Key=directory + "errors_cwop_{}.csv".format(end_api),
    )
