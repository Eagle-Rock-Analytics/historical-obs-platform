...
def get_madis_metadata(token, terrpath, marpath, networkid, bucket_name, directory):
    try:
        ...
        url = "https://api.synopticdata.com/v2/stations/metadata?token={}&network={}&bbox={}&recent=20&output=json".format(
        ...
        s3_cl.put_object(
            Bucket=bucket_name,
            Body=content,
            Key=directory + "stationlist_{}.csv".format(networkname),
        )
    ...
    for index, id in ids.iterrows():  # For each station
        ...
        if start_date is None:
            if id["start"] == "NaN" or pd.isna(
                ...
                start_api = id[
                    "start"
                ]  # Otherwise, set start date to start date of station.
        ...
        try:
            ...
            if options.get("timeout") == True:
                ...
                s3_obj = s3.Object(
                    bucket_name, directory + "{}_{}.csv".format(prefix, id["STID"])
                )
    ...
    for index, id in ids.iterrows():  # For each station
        ...
        try:
            ...
            if options.get("extension") is not None:
                if options.get("timeout") == True:
                    prefix = options.get("round")
                    filename = directory + "{}_{}_{}.csv".format(
                        prefix, id["STID"], options.get("extension")
                    )
                else:
                    filename = directory + "{}_{}.csv".format(
                        id["STID"], options.get("extension")
                    )
...
def madis_pull(token, networks=None, pause=None):
    if networks is None:  # If no networks provided, pull full list.
        ...
        networkdf = networkdf[
            networkdf["TOTAL_RESTRICTED"] == 0
        ]  # Remove restricted networks.
        networkdf = networkdf[
            ["ID", "SHORTNAME", "REPORTING_STATIONS"]
        ...
        resp = input(
            "Networks to be downloaded:{}. Would you like to continue? (Y/N)".format(
                list(networkdf["SHORTNAME"])
            )
        )
        if resp == "N":
            return
