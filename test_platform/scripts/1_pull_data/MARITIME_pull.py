...
years = list(
    map(str, range(1980, datetime.now().year + 1))
...
def get_maritime_station_ids(terrpath, marpath, directory_mar, directory_ndbc):
    ...
    weccgeo = gp.GeoDataFrame(
        stations, crs="EPSG:4326", geometry=geometry
...
def get_maritime(stations, bucket_name, network, years=None):
            ...
            for filename in dir_stations["STATION_ID"]:
                try:  # Try to get station txt.gz.
                    if (
                        ...
                        url = "https://www.meds-sdmm.dfo-mpo.gc.ca/alphapro/wave/waveshare/csvData/c{}_csv.zip".format(
                            filename
                        )  # all years in one file, including current year
                        s3_obj = s3.Object(
                            bucket_name, directory + "{}_csv.zip".format(filename)
                        )  # all years file format

                    else:  # NOAA NDBC Buoy archive
                        url = "https://www.ndbc.noaa.gov/data/historical/stdmet/{}h{}.txt.gz".format(
                            filename, year
                        )
                        s3_obj = s3.Object(
                            bucket_name,
                    ...
                    with requests.get(url, stream=True) as r:
                        if (
                            ...
                            print(
                                "Saving data for station {} for {}".format(
                                    filename, year
                                )
                            )
                        else:
            ...
            for filename in dir_stations["STATION_ID"]:
                try:
                    ...
                    while i >= start_month.month:
                        ...
                        x = current_date.strftime(
                            "%b"
                        )  # NDBC folder names is 3-letter month code
                        url = "https://www.ndbc.noaa.gov/data/stdmet/{}/{}{}{}.txt.gz".format(
                            x, filename, i, year
                        )
                        s3_obj = s3.Object(
                            bucket_name,
                            directory + "{}{}{}.txt.gz".format(filename, i, year),
                        )
    ...
    s3_cl.put_object(
        Bucket=bucket_name,
        Body=content,
        Key=directory + "errors_{}_{}.csv".format(network, end_api),
    )
...
def get_maritime_update(stations, bucket_name, network, start_date=None, end_date=None):
    ...
    ## Identifies which stations are owned by Canadian Dept of Environment and Climate Change (not qaqc'd by NDBC)
    canadian_owners = list(
        stations.loc[stations["OWNER"] == "C"]["STATION_ID"]
    )  # Canadian flag
            ...
            for filename in dir_stations["STATION_ID"]:
                try:  # Try to get station txt.gz.
                    if (
                        ...
                        s3_obj = s3.Object(
                            bucket_name,
                            directory + "{}_{}_csv.zip".format(filename, year),
                        )  # all years file format
            ...
            if filename in canadian_owners:
                ...
                try:
                    ...
                    if "January" in month_list:
                        prior_year_ind = month_list.index(
                            "January"
                    ...
                    for i in prior_year_months:
                        ...
                        url = "https://www.ndbc.noaa.gov/data/stdmet/{}/{}{}{}.txt.gz".format(
                            i, filename, month_nom, year
                        )

                        s3_obj = s3.Object(
                            bucket_name,
                            directory
                            + "{}{}{}.txt.gz".format(filename, month_nom, year),
                        )
