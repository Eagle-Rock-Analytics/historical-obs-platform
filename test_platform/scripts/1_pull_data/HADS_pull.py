"""
This script downloads HADS data from ISD using ftp.
Approach:
...
def get_hads_stations(terrpath, marpath): #Could alter script to have shapefile as input also, if there's a use for this.
    ...
    #Join and remove duplicates using GOES NESDIS ID as combined unique identifier.
    isdstations = (pd.concat([terwecc.iloc[:,:11], marwecc.iloc[:,:11]], ignore_index=True, sort =False)
    ...
    # Use output to check if names make sense intuitively.
    weccstations = weccstations.join(distances)
    weccstations = weccstations.join(indices)
    print(weccstations)
    #print(distances, indices)

    #print(isdstations.loc[indices])

    # Join to list of MADIS stations - network ID = 106.
    # Network IDs from HADS text file correspond to Station ID in MADIS.
