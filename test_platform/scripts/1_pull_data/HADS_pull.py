...
def get_hads_dat(bucket_name, directory, start_date=None, get_all=True):
    ...
    for i in years:
        try:
            yearurl = "https://www.ncei.noaa.gov/data/nws-hydrometeorological-automated-data-system/archive/{}/".format(
                str(i)
            )
    ...
    s3.put_object(
        Bucket=bucket_name,
        Body=content,
        Key=directory + "errors_hads_{}.csv".format(end_api),
    )
