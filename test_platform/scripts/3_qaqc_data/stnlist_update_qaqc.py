...
def get_qaqc_stations(network):
    ...
    for item in s3.Bucket(bucket_name).objects.filter(Prefix = network_prefix+network+"_"):
        if network == 'CW3E': # need to handle CW3E data differently in terms of qaqc check
            print('forthcoming')
        else: # all other networks
            qaqc_id = item.key.split("/")[-1].replace(".nc", "") # Get ID from file name
...
def qaqc_qa(network):
    if 'otherisd' in network: # Fixing capitalization issues
        ...
        if qaqc_ids.empty:
            print("No QA/QC'd files for this network. Please run the relevant qaqc script and try again.")
            exit()
        ...
        if 'ID_y' in stations.columns:
            stations['QAQC'] = np.where(stations.ID_y.isna(), "N", "Y") # Make binary qa/qc column
            # Drop ID column
            stations = stations.drop(['ID_x', "ID_y"], axis = 1)
        else:
            stations['QAQC'] = np.where(stations.ID.isna(), "N", "Y") # Make binary qa/qc column
            # Drop ID column
