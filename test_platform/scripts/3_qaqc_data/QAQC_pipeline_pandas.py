...
def process_output_ds(df, network, timestamp, station, qaqcdir,
    ...
    ds = ds.assign_attrs(history = 'ALLNETWORKS_qaqc_pandas.py script run on {} UTC'.format(timestamp))
...
#         filename = station + ".nc" # Make file name
        filename = station + "_pandas.nc" # Make file name
        ...
        os.system("mv {} temp/{}_pandas.nc".format(tmpFile.name, station))
            ...
            if file_name not in files: # dont run qa/qc on a station that isn't cleaned
                ...
                with fs.open(aws_url) as fileObj:
                    ...
                    # try:
                        ...
                        df = run_qaqc_pipeline(ds, network, file_name, errors, 
                        ...
                        if df is not None:
                            t0 = time.time()
                            process_output_ds(df, network, timestamp, station, qaqcdir, 
