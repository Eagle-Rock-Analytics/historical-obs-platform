...
def qaqc_world_record(ds, verbose=True):
    '''
...
def qaqc_crossvar_logic_tdps_to_tas(ds, verbose=True):
    """
    ...
    try:
        ...
        dew_vars = [col for col in ds.data_vars if 'tdps' in col]
...
def qaqc_crossvar_logic_calm_wind_dir(ds, verbose=True):
    """
    ...
    try:
        ...
        if 'sfcWind_dir' not in ds.data_vars:
            if verbose:
        ...
        if verbose:
            print('sfcWind_dir eraqc flags (any value other than nan is an active flag!): {}'.
                  format(np.unique(ds['sfcWind_dir_eraqc'])))
        return ds
