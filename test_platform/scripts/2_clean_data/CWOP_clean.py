...
def get_qaqc_flags(token, bucket_name, cleandir):
    ...
    s3_cl.put_object(Bucket=bucket_name, Body=content, Key=qaqcdir+"qaqcdict_CWOP.csv")
    return ids
...
def clean_cwop(bucket_name, rawdir, cleandir):
    ...
    #for i in ['AP156']: # For testing merge (on AWS)    
        ...
        ### TO DO? UNIT CHECK FOR ALL NETWORKS, or always feet????
        ...
        if 'sea_level_pressure_set_1_qc' in ds.keys(): # If QA/QC exists.
            if 'psl' in ds.keys():
                flagvals = list(np.unique(ds['sea_level_pressure_set_1_qc'].values))
                flagvals = list(np.unique([split_item for item in flagvals for split_item in str(item).split(";")])) # Split any rows with multiple flags and run unique again.
