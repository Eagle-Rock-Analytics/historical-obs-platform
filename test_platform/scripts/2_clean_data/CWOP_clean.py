...
def clean_cwop(bucket_name, rawdir, cleandir):
    try:
        ...
        for i in ids[-5:]: # Subsample for testing errors    
            try:
                ...
                station_id = "CWOP_"+i.upper() # Save file ID as uppercase always.
                headers = []
                    # Iterate through files to clean. Each file represents a station's data.
                for file in stat_files:
                    try:
                        header = parse_madis_headers(file, errors)
    ...
    # Write errors.csv
        ...
        content = csv_buffer.getvalue()
        s3_cl.put_object(Bucket=bucket_name, Body=content, Key=cleandir+"errors_CWOP_{}.csv".format(end_api))
