                    ...
                    with gzip.GzipFile(fileobj=obj.get()["Body"]) as gzipped_csv_file:
                        ...
                        for row in csv_reader: # Each row is a record
                            ...
                            if lat > latmax or lat < latmin or lon > lonmax or lon < lonmin:
                                ...
                                precip_len = re.search("(?<=AA1|AA2|AA3|AA4)[\da-zA-Z]{8}", string)
                                if precip_len is not None: # If precip data exists
                                    ...
                                    precip = precip_len.group() # Get the 8 strings after AA1/2/3/4
                                    ...
                                    if float(precip[2:6]) == 9999: # If precip depth of first report is missing.
                                        ...
                                        if precip is not None:
                                            precip = precip.group() # Get second set of precip values.
            ...
            if df.empty is False: # If there is data in the dataframe, convert to xarray object.
                try:
                    ...
                    if station_metadata['Barometer_elev'].isnull().all():
                        ...
                        ds = ds.assign_attrs(barometer_elev_m = barometer_height.values[0])
                    ...
                    if 'pr' in ds.keys():
                        ...
                        ds['pr'].attrs['ancillary_variables'] = "pr_qc pr_depth_qc pr_duration" # List other variables associated with variable (QA/QC)
                        ...
                        ds['pr_duration'].attrs['ancillary_variables'] = "pr pr_qc pr_depth_qc" # List other variables associated with variable (QA/QC)
