...
def clean_buoys(rawdir, cleandir, network):
    ...
    try:
        ...
        for station in stations.sample(5): # SUBST FOR TESTING
            station_metadata = station_file.loc[station_file['STATION_ID']==float(station)]
            ...
            for file in files: # For each zip file (annual or monthly)
                try:
                    ...
                    with BytesIO(obj.get()["Body"].read()) as tf:
                        ...
                        with gzip.open(file, "rt") as txt_file:
                            for index, row in txt_file:
                                # Get column names
                                if "#YY" in row:
                                    columns = row
                                    continue
                                ...
                                # Get row of data where values begins
                                if "#" not in row:
                                    first_row = index
                                    break
                                else:
                                ...
                                df = pd.read_csv(file, names=columns, header=first_row-2, low_memory=False)
...
if __name__ == "__main__":
    rawdir, cleandir, qaqcdir = get_file_paths("NDBC")
    print(rawdir, cleandir, qaqcdir) # TESTING
    clean_buoys(rawdir, cleandir, network="NDBC")
