...
for s in range(len(wecc_df)):
    ...
    fname = 'wecc-hadisd3.3.0.202202p/hadisd.3.3.0.202202p_19310101-20220301_'+f_id+'.nc'
    ...
    for i in range(len(reporting_vars)):
        ...
        missing_obs[np.where(ds_infilled[var].values==ds_infilled[var].attrs['missing_value']),i] = 1.
        missing_obs[np.isnan(ds_infilled[var].values)] = 1.
    no_report = xr.DataArray(data=missing_obs,coords={'time' : ds_infilled["time"], 'reporting_v' : ds_infilled['reporting_v']})
    ...
    for i in range(len(the_vars)):
        ...
        try:
            ...
            if (isinstance((tocheck), (int, np.integer))):                       
                ds_infilled[var] = ds_infilled[var].where(ds_infilled[var]>int(-887)).fillna(int(-999)).astype(int)
            elif (isinstance((tocheck), (float))):
                ds_infilled[var] = ds_infilled[var].where(ds_infilled[var]>-1e30).fillna(-1e30).astype(float)
            elif (isinstance((tocheck), (np.bytes_))):
                ds_infilled[var] = ds_infilled[var].fillna('no report').astype(str)              
        except:
            print(var)

    arr[np.where(np.isnan(ds_infilled['quality_control_flags'].values[:,0]))] = 1.
